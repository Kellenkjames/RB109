...
The `puts` method invocation passes the value `a` to it as an argument on line 13. Since `a` is now assigned to `c` and `c` is assigned to the Integer `3`; this is what is output; the method returns `nil`.

The `puts` method invocation passes the value `b` to it as an argument on line 14. Since `b` is not reassigned anywhere in the program; it will output the Integer `2`; the method returns `nil`.
